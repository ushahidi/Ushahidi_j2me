...
public class Helper {
    ...
    private static StreamReader getStreamReaderPrim(String name)
            throws UnsupportedEncodingException {
        try {
            return (StreamReader) getStreamReaderOrWriter(name,
                    "_Reader");
    ...
    private static Object getStreamReaderOrWriter(String name,
    ...
    private static StreamWriter getStreamWriterPrim(String name)
            throws UnsupportedEncodingException {
        try {
            return (StreamWriter) getStreamReaderOrWriter(name,
                    "_Writer");
        } catch (ClassCastException x) {
            throw new RuntimeException(
            /* #ifdef VERBOSE_EXCEPTIONS */
    ...
    public static char[] byteToCharArray(byte[] buffer, int offset,
            int length) {
        try {
            return byteToCharArray(buffer, offset, length,
                    defaultEncoding);
        } catch (UnsupportedEncodingException x) {
            throw new RuntimeException(
            /* #ifdef VERBOSE_EXCEPTIONS */
    ...
    public static byte[] charToByteArray(char[] buffer, int offset,
            int length) {
        try {
            return charToByteArray(buffer, offset, length,
                    defaultEncoding);
        } catch (UnsupportedEncodingException x) {
            throw new RuntimeException(
            /* #ifdef VERBOSE_EXCEPTIONS */
     ...
     * @exception UnsupportedEncodingException  If the encoding is not known
     */
    public static synchronized char[] byteToCharArray(byte[] buffer,
        ...
        if (lastReaderEncoding == null
                || !lastReaderEncoding.equals(enc)) {
        ...
        lastReader.open(
                new ByteArrayInputStream(buffer, offset, length), enc);
     ...
     * @exception UnsupportedEncodingException  If the encoding is not known
     */
    public static synchronized byte[] charToByteArray(char[] buffer,
            int offset, int length, String enc)
            throws UnsupportedEncodingException {
        ...
        if (lastWriterEncoding == null
                || !lastWriterEncoding.equals(enc)) {
    ...
    private static String normalizeEncodingName(String encodingName) {
        ...
        for (int i = 0; i < normalizedName.length(); i++) {
            ...
            if (currentChar == '-' || currentChar == ':') {
                ...
                normalizedName.setCharAt(i, Character
